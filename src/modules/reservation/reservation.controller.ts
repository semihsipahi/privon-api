...
export class ReservationController {
  ...
  @ApiOperation({
    ...
    description:
      'Kullanıcının tamamlanmış rezervasyonlardan elde ettiği toplam tasarrufu ve detayları döner.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Tamamlanmış rezervasyonları paginasyonlu listeler. Kullanıcı adı, indirim %, kazanç, restoran adı ve tarih bilgileri döner.',
  })
  @ApiQuery({
    name: '_start',
    required: false,
    description: 'Başlangıç index (default: 0)',
  })
  @ApiQuery({
    name: '_end',
    required: false,
    description: 'Bitiş index (default: 10)',
  })
  @ApiQuery({
    name: '_sort',
    required: false,
    description: 'Sıralama alanı (default: date)',
  })
  @ApiQuery({
    name: '_order',
    required: false,
    description: 'Sıralama yönü: asc/desc (default: desc)',
  })
  @ApiQuery({
    name: 'restaurantId',
    required: false,
    description: 'Restoran ID ile filtrele',
  })
  @ApiQuery({
    name: 'startDate',
    required: false,
  ...
  @ApiQuery({
    name: 'customerName',
    required: false,
    description: 'Müşteri adı ile filtrele',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Belirli bir kullanıcının tüm rezervasyon geçmişini ve finansal istatistiklerini (toplam harcama, tasarruf vb.) döner.',
  })
  @ApiQuery({
    name: '_start',
    required: false,
  ...
  async getUserSummary(@Param('userId') userId: string, @Query() query: any) {
    return await this.reservationService.getUserReservationSummary(
      userId,
      query,
    );
  }
  ...
  @Roles(Role.SuperAdmin)
  @ApiOperation({
    summary: 'Phantom Rezervem kayıtlarını sil (onay kodu olmayan)',
  })
