...
> {
  ...
  async createReview(
    ...
    if (reservation.customer.toString() !== user.userId) {
      throw new ForbiddenException(
        'Sadece kendi rezervasyonunuzu değerlendirebilirsiniz',
      );
    }
    ...
    if (existingReview) {
      throw new BadRequestException(
        'Bu rezervasyon için zaten değerlendirme yapılmış',
      );
    }
  ...
  async replyToReview(id: string, replyDto: ReplyReviewDto, user: AuthUser) {
    ...
    if (!review.isActive) {
      throw new BadRequestException(
        'Sadece onaylanmış değerlendirmelere yanıt verilebilir',
      );
    }
    ...
    if (user.role !== Role.SuperAdmin) {
      if (
        !user.restaurantId ||
        user.restaurantId !== review.restaurant.toString()
      ) {
        throw new ForbiddenException(
          'Sadece kendi restoranınıza ait değerlendirmelere yanıt verebilirsiniz',
        );
      }
    }
